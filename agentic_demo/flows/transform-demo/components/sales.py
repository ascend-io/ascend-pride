...
def sales(
    ...
    stores_cols = ["STORE_ID", "ASCENDER_ID", "PRICE", "QUANTITY", "TIMESTAMP", "ID"]
    website_cols = ["ASCENDER_ID", "ID"]
    vendors_cols = ["ID", "VENDOR_ID"]
    ...
    joined = stores_web.join(
        sales_vendors_sel, stores_web.ID == sales_vendors_sel.ID, how="left"
    )[
    ...
    daily_sales = joined.group_by(
        [joined.STORE_ID, joined.TIMESTAMP.date().name("SALE_DATE")]
    ).aggregate(total_sales=(joined.PRICE * joined.QUANTITY).sum())
